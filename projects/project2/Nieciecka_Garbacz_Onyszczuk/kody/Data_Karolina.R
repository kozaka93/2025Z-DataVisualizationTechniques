...
## Wczytywanie danych --------------------------------------------------------- 
phone_data <- read.csv("Data_csv/dane_Karolina/Karolina_telefon.csv")
laptop_data <- read.csv("Data_csv/dane_Karolina/Karolina_komputer_afk.csv")
...
# Alarm clock -----------------------------------------------------------------
morn <- phone_data %>% 
  filter(!(phone_app %in% c('Ekran startowy One UI', 'Aktualizacja oprogramowania'))) %>% # dane systemowe
...
alarm_clock <- segments %>% 
  filter(phone_app == 'Zegar', segment_number == 0) %>% 
  group_by(date) %>% 
...
sleep <- sleep[-1,] # -Inf w pierwszej linijce
...
# Zapis csv -------------------------------------------------------------------
# write.csv(screentime, "screentime_karolina.csv", row.names = FALSE)
# write.csv(sleep, 'sleep_karolina.csv', row.names = FALSE)
