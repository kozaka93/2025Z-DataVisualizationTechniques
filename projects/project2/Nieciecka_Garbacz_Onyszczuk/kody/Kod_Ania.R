...
## Wczytywanie danych --------------------------------------------------------- 
phone_data <- read.csv("Data_csv/dane_Ania/Ania_telefon.csv")
laptop_data <- read.csv("Data_csv/dane_Ania/Ania_afk_komputer.csv")
...
# Alarm clock -----------------------------------------------------------------
morn <- phone_data %>% 
  filter(!(phone_app %in% c('POCO Launcher', 'Aktualizacje aplikacji systemowych'))) %>% # dane systemowe
...
# Zapis csv -------------------------------------------------------------------
# write.csv(screentime, "screentime_ania.csv", row.names = FALSE)
# write.csv(sleep, 'sleep_ania.csv', row.names = FALSE)
