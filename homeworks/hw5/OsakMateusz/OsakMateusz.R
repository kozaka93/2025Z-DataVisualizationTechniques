...
p <- ggplot() +
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5, color = "#FFD700", face = "bold", size = 24, margin = margin(b = 10))
  ) +
  ggtitle("WESOŁYCH ŚWIĄT!!!") +
  theme(plot.title = element_text(hjust = 0.5, color = "white", face = "bold")) +
  transition_manual(frame)

anim <- animate(p, nframes = n_frames, fps = 10, width = 450, height = 550, renderer = gifski_renderer())
